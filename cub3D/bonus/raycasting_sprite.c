...
/*   Updated: 2021/04/19 20:57:02 by erecuero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	setup_sprites(t_game *game, t_ray *ray, t_sprite *sprite, int i)
{
	...
	game->sprite.inv_det = 1.0 / (ray->plane.x * ray->dir.y - ray->dir.x *
						ray->plane.y);
	game->sprite.transform.x = game->sprite.inv_det * (ray->dir.y * game->sprite.s_pos.x
							- ray->dir.x * game->sprite.s_pos.y);
	game->sprite.transform.y = game->sprite.inv_det * (-ray->plane.y * game->sprite.s_pos.x
							+ ray->plane.x * game->sprite.s_pos.y);
	game->sprite.s_screenx = (int)((game->set.res.x / 2) * (1 + game->sprite.transform.x
