...
char	is_wall_raycasting(t_game *game, t_axis next_touch)
{
	...
	map = game->set.map;
	res = game->set.res;
	index_x = (int)(next_touch.x);
	index_y = (int)(next_touch.y);
	if (index_x < 0 || index_y < 0
		|| index_x >= res.x - 1 || index_y >= res.y - 1 || index_y > game->set.map_height || index_x > (int)ft_strlen(game->set.map[index_y]))
		return ('1');
	else
...
int		is_wall(t_game *game, float x, float y)
{
	...
	index_y = floor(y);
	if (index_x < 0 || index_y < 0
		|| index_x >= res.x || index_y >= res.y || index_y > game->set.map_height || index_x > (int)ft_strlen(game->set.map[index_y]))
		return (1);
...
void	update_player_position(t_game *game, t_player *player)
{
	...
	next_position.x = player->pos.x + (xstep * move_step);
	next_position.y = player->pos.y + (ystep * move_step);
